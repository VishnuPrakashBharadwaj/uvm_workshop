...
task ahb_master_driver::run_phase(uvm_phase phase);
    ...
    begin
		...
		@(posedge vif.HCLK) begin
			vif.HADDR <= req.m_address;				// Drive the address
			if(req.m_read_write == AHB_READ)		
				vif.HWRITE <= 1'b0;					
			else if(req.m_read_write == AHB_WRITE)	
				vif.HWRITE <= 1'b1;					
		end
	
		// Data Phase
		// Checking for read instruction
		...
		if(req.m_read_write == AHB_READ) begin
			vif.HRDATA = req.m_rdata;		// Reading the data	
		end else if (req.m_read_write == AHB_WRITE) begin
			vif.HWDATA = req.m_wdata;		// Writing the data
		end
	
		@(posedge vif.HCLK);
		vif.HRESP = req.m_response_type;		// Reading the response
